...
- **Matériel très mince** : 9 fichiers, dont 4 utiles — page de structure StudiUM, les deux devoirs, le corrigé manuscrit du devoir 1 (10 p.), et une page scannée de tableaux d'équivalences logiques. **Aucune note de cours, aucun quiz relu, aucun ancien examen, aucun exercice de TP.**
- **Calendrier confirmé** : intra le **jeudi 29 octobre, 10h30-12h20**, B-2325 Pav. 3200 J.-Brillant, 35 %, **chapitres 1 à 3** ; final le 10 décembre, cumulatif, 55 % ; 5 quiz de 15 min en TP (2 % chacun), pris « directement des deux derniers devoirs ». L'intra et les quiz occupent la plage du TP du jeudi. **Aucun calendrier de matière semaine par semaine n'existe** : le seul tableau daté est celui des évaluations — écart de méthode net avec MAT1400 et MAT1600.
- **`exam` des 10 thèmes : aucun écart, rien à reclasser, aucun id à retirer.** Les 8 thèmes intra couvrent exactement les chapitres 1 à 3, les 2 thèmes final les chapitres 4-5 et 7.
- **Un seul `defaultOn` en écart** : `mat1500-proof` était `false` alors que le devoir 1 #1-#2 (deux preuves à rédiger, la seconde explicitement « pour contradiction ») et le devoir 2 #5-#6 (contre-exemples, réciproque, contraposée) sont exigibles au 17 septembre. Corrigé à `true`. Les neuf autres sont justes.
- **Une affirmation du jalon 1 était fausse** : `docs/sources/mat1500.md` disait les devoirs 1-2 « entièrement de la logique propositionnelle et des quantificateurs ». Non : deux des neuf problèmes du devoir 1 sont des preuves sur les **arbres**, et le devoir 2 #6 porte sur la parité et la divisibilité. Le fichier est refait à partir des sources réelles.
- **Piège signalé** : le devoir 1 commence par de la théorie des graphes (chapitre 7, matière du **final**), utilisée comme terrain de rédaction de preuve. Comme le quiz prend ses questions dans les devoirs, cette matière est examinable au quiz du 17 septembre et la banque n'a rien dessus. `mat1500-graphs` reste `exam: 'final'` sans question ; le fait est consigné.
- **6 gaps réels comblés, 6 questions** (aucun Défi), chacun rattaché à un problème précis : **« est-ce une proposition ? »** 0 question sur 96 alors que c'est le plus long problème du devoir 1 → `logic-013` (qcm) et `logic-014` (flash, introduit le mot « prédicat ») ; **parenthésage d'une expression mixte $\wedge$/$\vee$** (devoir 2 #5a-b, morale écrite dans l'énoncé) → `logic-015` ; **négation du biconditionnel** (devoir 2 #3e) → `logic-016` ; **$\forall$ ne distribue pas sur $\vee$** (devoir 2 #5c ; la banque avait les deux cas du $\exists$, pas celui-là) → `quant-013` ; **« suffit pour »** (devoir 2 #1e-f ; seul « nécessaire » était couvert) → `proof-013`.
- **Les 96 questions existantes : 0 modifiée sur le fond, 0 retirée, 0 id à retirer.** Là où le recoupement est possible il est exact ($\exists!$ du devoir 1 #9, réfutation du « plus petit réel » par le même argument que le corrigé, $\neg(p\to q)$ du devoir 2 #3b, exportation du devoir 2 #4e, lignes des tableaux 5 et 6). Une seule `explanation` retouchée (`logic-010` : le manuel note les constantes de vérité $V$ et $F$) — coquille de notation, id conservé.
- **Notation vérifiée, rien à réécrire** : $\Leftrightarrow$ (devoirs, tableaux français) et $\equiv$ (tableaux anglais) figurent tous deux sur la page remise aux étudiants ; majuscules $P,Q,R$ dans les devoirs contre minuscules du manuel, la banque suit le manuel. À noter : **ni « absorption » ni « exportation » ne sont nommées** dans le tableau du manuel français, alors que `logic-011` et `logic-012` s'appuient sur ces noms (qu'ils donnent eux-mêmes dans l'énoncé). `sets-011` ($A \oplus B$, « notation de Rosen ») est **invérifiable** : aucun matériel sur les ensembles.
- Brouillon des 6 questions écrit à la main, **sans agy** : les PDF étaient lisibles directement (texte intégré, corrigé manuscrit lisible) et le calibrage exigeait d'avoir lu tout le matériel. Gate d'import (`npm run import -- <fichier> --renumber`) et relecture aveugle faits comme prévu.
- Relecture aveugle des 6 nouvelles questions (`scripts/blind-review.ts`, seed 17, clé jamais montrée, sous-agent interdit de lire le dépôt) par un sous-agent **Haiku** frais : **6/6 confirmées, 0 désaccord, 0 ambiguïté**.
- **Non vérifié, à rouvrir** : les titres de section du manuel ne sont nulle part dans `D:\Math` (si la section 2.1 porte sur les algorithmes, c'est de la matière d'intra sans aucune question) ; les 5 thèmes `sets`, `func`, `divis`, `modul`, `induc` (60 questions) n'ont pu être confrontés à **rien** ; sans quiz ni ancien examen, le niveau de difficulté attendu reste inconnu.
- **Compte mesuré : 96 → 102 questions MAT1500** (+6, 0 retirée). `mat1500-logic` 12→16, `mat1500-proof` 12→13, `mat1500-quant` 12→13. Équilibre : Défi 14/102 = 13,7 %, qcm 61/102 = 59,8 %. `npm test` : 363 tests verts, `tsc --noEmit` : OK.
...
- **Calendrier confirmé** : intra le **lundi 26 octobre, 15h30-17h20**, matière « annexes A et B ; chapitres 3, 4 et 5 » (citée deux fois sur StudiUM : calendrier et forum de consignes). Les 12 dates de séance de `index.ts` correspondent une à une au calendrier officiel, y compris l'ordre inhabituel 4.4 **avant** 4.1-4.3.
- **`exam` des 12 thèmes : aucun écart, rien à reclasser, aucun id à retirer.** Contrairement à MAT1600, les 8 thèmes intra couvrent exactement la matière citée et les 4 thèmes final les chapitres enseignés après le 26 octobre. Les 96 questions existantes ont été relues une à une contre l'aide-mémoire de l'intra (feuille de formules remise à l'examen), la liste des TP, le quiz relu et les anciens examens : **0 modifiée, 0 retirée**.
- **Un seul `defaultOn` en écart** : `mat1400-grad` était `false` alors que la théorie du chap. 4.4 a lieu le jeudi 17 septembre 8h30-10h30, donc avant la date du build. Corrigé à `true`. Libellé de `mat1400-tint` complété (le chap. 7 va jusqu'à 7.5, changement de variables) ; en-tête de `index.ts` re-sourcé sur la page de structure StudiUM.
- **Piège signalé** : les 5 anciens intras ne sont **pas représentatifs** de l'intra A26. Chacun consacre ses questions 1 et 2 aux séries numériques et aux séries de Taylor (chap. 1 et 2), qui en A26 sont enseignés en novembre et évalués au **final** ; aucun ne contient de question sur les quadriques, le gradient, l'optimisation ou Lagrange. Ce qui reste transférable : droites et plans (5 fichiers sur 6), limites/continuité (6 sur 6), plan tangent et différentiabilité (4 sur 6).
- **3 gaps réels comblés, 11 questions**, chacun confirmé par au moins deux sources indépendantes : **équations paramétriques de droites** 0 question sur 96 alors que le seul quiz rapatrié porte entièrement dessus, que la feuille de formules donne $\vec r(t)=\vec r_0+t\vec v$ et que 5 anciens intras sur 6 ont une question complète bâtie dessus → +6 (`mat1400-vect-013` à `-018`, dont 1 Défi) ; **différentiabilité** réduite à 2 Vrai/Faux alors que c'est le thème 4 du cours sur StudiUM et un bloc entier de la feuille de formules → +4 (`mat1400-part-013` à `-016`) ; **fonction auxiliaire $F=f-\lambda g$**, l'écriture de la feuille de formules, absente du thème Lagrange → +1 (`mat1400-lagr-015`).
- Dépouillement des 5 anciens intras + corrigé de révision A24 délégué à **agy** (quota Google), puis vérifié par transcription verbatim de 4 questions clés avant d'agir dessus. Brouillon des 11 questions écrit à la main (le skill agy exclut les maths).
- Relecture aveugle des 11 nouvelles questions (`scripts/blind-review.ts`, seed 23, clé jamais montrée, sous-agent interdit de lire le dépôt) par un sous-agent **Haiku** frais : **11/11 confirmées, 0 désaccord, 0 ambiguïté**.
- **Écarts sur le matériel lui-même** (rien à corriger dans le dépôt) : `05 Quiz & solutions` ne contient **qu'un seul fichier**, pas 16 comme l'annonçait le brief — les thèmes 2 à 6 n'ont aucune relecture de quiz ; le forum de consignes d'examen annonce encore l'intra « lundi 23 février » (reliquat H26, le calendrier et le barème disent tous deux le 26 octobre) ; le PDF des TP porte l'en-tête « Automne 2025 » ; corrigés de TP et anciens examens présents en double (curation manuelle, rien déplacé).
- **Compte mesuré : 96 → 107 questions MAT1400** (+11, 0 retirée). `mat1400-vect` 12→18, `mat1400-part` 12→16, `mat1400-lagr` 12→13. Équilibre : Défi 9/107 = 8,4 %, qcm 67/107 = 62,6 %. `npm test` : 363 tests verts, `npm run build` : OK.
